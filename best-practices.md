...
这样就生成了一份仓颉基础语法书的记忆， 在新项目或新会话中，使用提示词让ai加载就行了`（这一步只做一次就行，一个cangjie-mem环境是可持久且多项目共用的，http版本还可以跨机器团队使用）`
...
或者同时指定需求
```md
 使用cangjie-mem list加载所有的仓颉语言级记忆, 并读取设计文档，和任务列表，然后按任务生成实现代码`（需要先写好设计文档）
```
